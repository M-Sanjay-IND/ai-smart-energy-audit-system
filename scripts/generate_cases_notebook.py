...
def create_notebook():
    ...
    # ── Cell 2: Imports & Inline Model Training (self-contained)
    mk_code(
        "import numpy as np\n"
        "import pandas as pd\n"
        ...
        "try:\n"
        "    df_train = pd.read_csv('../backend/raw_data.csv')\n"
        "    dft = df_train.copy()\n"
        ...
        "    # ── Build & train Random Forest inline\n"
        "    df_rf = dft.copy()\n"
    ...
    # ── Cell 3: evaluate_and_plot helper
    mk_code(
        ...
        "    iso_features = dfd[['delta_v','delta_i','delta_p','hist_mean_p_10','hist_mean_p_50']].values\n"
        "    rf_features = dfd[['delta_v','delta_i','delta_p','lag1_delta_p','lag2_delta_p','lag3_delta_p','hist_mean_p_10','hist_mean_p_50']].values\n"
        "\n"
        "    anomalies = np.zeros(len(dfd), dtype=bool)\n"
        "    forecast_power = np.zeros(len(dfd))\n"
        "    if iso_model is not None:\n"
        "        try:\n"
        "            iso_scaled = iso_scaler.transform(iso_features)\n"
        ...
        "        try:\n"
        "            rf_scaled = rf_scaler.transform(rf_features)\n"
        "            predicted_p = rf_model.predict(rf_scaled)\n"
        "            forecast_power = np.roll(predicted_p, 1)\n"
        "            forecast_power[0] = dfd['power'].iloc[0]\n"
        "        except Exception as e:\n"
        "            print(f'  ⚠️  RF Inference error: {e}')\n"
        "    dfd['is_anomaly'] = anomalies\n"
        "    dfd['forecast_power'] = forecast_power\n"
        "    idx = dfd.index\n"
        "\n"
        "    # ── Clean line-chart style ──\n"
        "    fig, axes = plt.subplots(4, 1, sharex=True, figsize=(14, 13))\n"
        ...
        "    ]\n"
        "    for i, (color, ylabel, col) in enumerate(styles):\n"
        "        axes[i].plot(idx, dfd[col], color=color, lw=1.4, alpha=0.9, label=ylabel)\n"
        "        # Mark anomaly points with red stars only\n"
        "        if col == 'power':\n"
        "            adf = dfd[dfd['is_anomaly']]\n"
        "            if not adf.empty:\n"
        "                axes[i].scatter(adf.index, adf[col],\n"
        "                           marker='*', s=200, color='#e74c3c',\n"
        "                           zorder=5, label='Anomaly (IF)', edgecolors='#c0392b', linewidths=0.5)\n"
        "        axes[i].set_ylabel(ylabel, fontsize=10)\n"
        "        axes[i].grid(True, alpha=0.25, linestyle='--')\n"
        "        axes[i].legend(loc='upper right', fontsize=9)\n"
        "\n"
        "    # RF Subplot\n"
        "    axes[3].plot(idx, dfd['power'], color='#27ae60', lw=1.2, alpha=0.4, label='Actual Power')\n"
        "    axes[3].plot(idx, dfd['forecast_power'], color='#9b59b6', lw=1.4, alpha=0.9, label='RF Forecast Power')\n"
        "    axes[3].set_ylabel('Forecast (W)', fontsize=10)\n"
    ...
    mk_md("### Conclusion: All Cases Combined\n"
          "A single visual summary chart comprising all four cases across voltage, current, real power, and RF predicted dimensions.")
    mk_code("fig, axes = plt.subplots(4, 4, figsize=(22, 16))\n"
            ...
            "]\n\n"
            "cols_info = [('voltage','#4a90d9','Volts'), ('current','#e67e22','Amps'), ('power','#27ae60','Watts'), ('forecast_power','#9b59b6','RF Forecast (W)')]\n\n"
            "for i, (name, df_case) in enumerate(datasets):\n"
            "    for j, (col, color, unit) in enumerate(cols_info):\n"
            "        ax = axes[i, j]\n"
            "        if col == 'forecast_power':\n"
            "            ax.plot(df_case.index, df_case['power'], color='#27ae60', lw=1.2, alpha=0.3)\n"
            ...
            "        ax.set_title(f'{name} — {col.capitalize().replace(\"_\",\" \")}', fontsize=9, fontweight='bold')\n"
    ...
    output_path = os.path.join(os.path.dirname(__file__), "..", "docs", "case_studies_evaluation.ipynb")
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(nb_dict, f, indent=2)
    print(f"[SUCCESS] Successfully written notebook file: {output_path}")
