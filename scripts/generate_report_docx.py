...
def train_and_evaluate_rf(df: pd.DataFrame):
    """Train absolute power Random Forest Regressor, return evaluation metrics."""
    dfd = _compute_deltas(df)
    ...
    dfd = dfd[dfd["delta_p"].abs() < 100] # remove anomaly boundary jumps
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "docs", "energy_audit_statistical_report.docx")
    build_docx(
        df, if_stats, if_scores, if_preds,
        rf_stats, rf_y_test, rf_y_pred,
