...
RAW_DATA_PATH = os.path.join("..", "backend", "raw_data.csv")
...
def create_features(df):
    ...
    # Lag features
    df['lag1_delta_p'] = df['delta_p'].shift(1).fillna(0)
    df['lag2_delta_p'] = df['delta_p'].shift(2).fillna(0)
    df['lag3_delta_p'] = df['delta_p'].shift(3).fillna(0)
    ...
    # Target is predicting the NEXT absolute power
    df['next_p'] = df['power'].shift(-1)
    
    df.dropna(inplace=True)
    return df

def train_model(df):
    feature_cols = ['delta_v', 'delta_i', 'delta_p', 'lag1_delta_p', 'lag2_delta_p', 'lag3_delta_p', 'hist_mean_p_10', 'hist_mean_p_50']
